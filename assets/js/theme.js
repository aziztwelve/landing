...
(function($) {
    ...
    $('.expand-video').magnificPopup({
        ...
        callbacks: {
            open: function () {
                //$('#message').val(this.st.el.attr('data-price') + this.st.el.attr('data-tree-name'));
                //console.log(this.st.el.attr('data-price') + this.st.el.attr('data-tree-name'));
                //window.top.jQuery('#message').val(this.st.el.attr('data-price'));
            },
        }
    });

    $('.expand-video').on('click', function () {
        var name = $(this).attr('data-tree-name') + ' ' + $(this).attr('data-price');
        console.log('Tree name:' + $(this).attr('data-tree-name'));
        console.log('Price: ' + $(this).attr('data-price'));
        $('#fullNameTree').val(name);
        setTimeout(function () {
            var fullNameTree = $('#fullNameTree').val();
            $('iframe.mfp-iframe').contents().find('#message').val(fullNameTree)
        }, 2000);
    });
